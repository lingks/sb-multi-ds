...
public class MybatisConfiguration extends MybatisAutoConfiguration
{
    ...
    public AbstractRoutingDataSource dynamicDataSource()
    {
        ...
        targetDataSources.put(DataSourceKey.slaveDataSource.name(), slaveDataSource);
        targetDataSources.put(DataSourceKey.slave2DataSource.name(), slave2DataSource);

        proxy.setDefaultTargetDataSource(masterDataSource);
        proxy.setTargetDataSources(targetDataSources);

//        Iterator<Object> iterator = targetDataSources.keySet().iterator();
//        while(iterator.hasNext()){
//            DataSourceContextHolder.slaveDataSourceKeys.add(iterator.next());
//        }
        targetDataSources.forEach((k,v)->{
            DataSourceContextHolder.addSlaveDataSource(k.toString());
        });
